...
type QuizResultProps = {
  height: number
  desiredIncrease: number // Add desiredIncrease prop
  onContinue: () => void
}

export default function QuizResult({ height, desiredIncrease, onContinue }: QuizResultProps) {
  const potentialGain = desiredIncrease
  const currentLevel = height // Reutilizamos el parámetro height como nivel actual
  const maxLevel = currentLevel + potentialGain
        ...
        >
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold text-white">
              Puedes aumentar hasta <span className="text-red-500 text-3xl">{potentialGain}%</span> tu testosterona
            </h2>
